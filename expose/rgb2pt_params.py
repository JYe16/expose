...
def main(
    ...
    image_folder = osp.join(root_folder, 'img')
    params_output_folder = osp.join(root_folder, 'params')
    mesh_output_folder = osp.join(root_folder, 'mesh')
    base_joint_output_folder = osp.join(root_folder, 'base_joint')
    ...
    os.makedirs(base_joint_output_folder, exist_ok=True)
    ...
    for bidx, batch in enumerate(tqdm(expose_dloader, dynamic_ncols=True)):
        ...
        for idx in tqdm(range(len(body_targets)), 'Saving ...'):
            ...
            curr_out_path = osp.join(mesh_output_folder, fname)
            ...
            if save_params:
                ...
                params_fname = osp.join(params_output_folder, f'{filename}_params.npz')
    ...
    if save_mesh:
        mesh_pt_name = osp.join(mesh_output_folder, image_folder.split('/')[-2] + '_mesh.pt')
        torch.save(torch.FloatTensor(mesh_points), mesh_pt_name)
    if save_base_joint:
        pt_name = image_folder.split('/')[-1] + '_base_joint.pt'
        torch.save(base_joint, base_joint_output_folder + pt_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_folder', type=str, dest='root_folder',
                        default='/mnt/h/Datasets/Own/WiMesh-X/0606-yzk-write/',
                        help='The root folder containing all data')
    ...
    parser.add_argument('--save-base-joint', dest='save_base_joint', default=True,
                        type=lambda x: x.lower() in ['true'],
                        help='Whether to save the base 144 joints')

    cmd_args = parser.parse_args()

    root_folder = cmd_args.root_folder
    show = cmd_args.show
    ...
    save_base_joint = cmd_args.save_base_joint
    ...
    with threadpool_limits(limits=1):
        main(
            ...
            save_vis=save_vis,
            save_mesh=save_mesh,
            save_base_joint = save_base_joint,
