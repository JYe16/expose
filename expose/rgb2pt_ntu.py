...
def main(
    ...
    mesh_pt_name = '/media/persec/Data/yzk/ntu_pt/'
    if define_training_data(image_folder):
        mesh_pt_name += 'train/'
    else:
        mesh_pt_name += 'test/'
    image_folder_splited = image_folder.split('/')
    pt_name = image_folder_splited[len(image_folder_splited) - 1]
    mesh_pt_name += pt_name + '.pt'
    torch.save(torch.FloatTensor(mesh_points), mesh_pt_name)
    print(f"Generated pt file {image_folder}.pt. Shape {mesh_points.shape}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-folder', type=str, dest='image_folder',
                        default='20230322-head-rotation',
